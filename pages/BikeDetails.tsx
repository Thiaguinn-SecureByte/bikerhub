...
const BikeDetails: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      {/* Navigation & Header */}
      <div>
        <Link to={`/brand/${brandId}/model/${modelId}`} className="inline-flex items-center text-slate-400 hover:text-white mb-4 transition-colors text-sm">
          <ArrowLeft className="w-4 h-4 mr-1" /> Volver a años
        </Link>
        <div className="flex flex-col md:flex-row md:items-end gap-4">
          <h1 className="text-4xl md:text-5xl font-extrabold text-white">
            {brand.name} <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">{model.name}</span>
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              {loading ? (
                  Array(5).fill(0).map((_, i) => (
                      <div key={i} className="bg-slate-800 h-24 rounded-xl animate-pulse" />
                  ))
              ) : (
                  <>
